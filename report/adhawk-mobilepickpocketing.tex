...
An equally effective, less flammable option is to crush the RFID chip inside the card.  While generally safer than the microwave option, it requires better aim.  The RFID chip can be tricky to find.  See Figure~\label{fig:RFID_in_PNC_Card} and Figure~\label{fig:RFID_chip_location} for details on the location of the PNC payWave’s RFID chip.  Once the chip is identified, disabling the chip is simple: smashing it with a hammer should render the chip useless \cite{threat-analysis-of-rfid-passports}.  Once destroyed, the RFID chip will no longer communicate any data whatsoever, ensuring perfect data privacy.
